...
class  MCP_API MediapipeHolistTrack
{
public:
	...
	*/
	...
	@return 返回操作成功或者失败
	*/
	bool DetectImage(int image_width, int image_height, void* image_data, std::map<std::string, std::vector<float>> &holist_result, bool show_result_image = false);
...
private:
	...
	absl::Status Mediapipe_RunMPPGrapht(int image_width, int image_height, void* image_data, std::map<std::string, std::vector<float >> &holist_result, bool show_result_image = false);
